...
fn create_session_request(filename: &str, file_size: u64) -> Request<Body> {
    let body = json!({
        "filename": filename,
        "file_size": file_size,
    })
...
async fn create_session_returns_a_one_time_code() {
    ...
    let response = send(&app, create_session_request("demo.bin", 1024)).await;
...
async fn create_session_rejects_payloads_over_limit() {
    ...
    let response = send(
        &app,
        create_session_request("large.bin", MAX_UPLOAD_SIZE_BYTES + 1),
    )
    .await;
...
async fn create_session_rate_limits_after_ten_requests_per_minute() {
    ...
    for index in 0..10 {
        let response = send(
            &app,
            create_session_request(&format!("file-{index}.bin"), 1024),
        )
        .await;
    ...
    let response = send(&app, create_session_request("blocked.bin", 1024)).await;
